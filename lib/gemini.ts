...
- Primary Concerns: ${input.primaryConcerns.join(', ')}
...
Return ONLY valid JSON matching this exact structure with no markdown wrapping.`;
      ...
      return {
        morningRoutine: parsed.morningRoutine.map((step: any) => ({
          ...step,
          recommendedProduct: catalogProducts.find((p) => p.id === step.recommendedProductId || p.category === step.productCategory),
        })),
        eveningRoutine: parsed.eveningRoutine.map((step: any) => ({
          ...step,
          recommendedProduct: catalogProducts.find((p) => p.id === step.recommendedProductId || p.category === step.productCategory),
        })),
        expertAdvice: parsed.expertAdvice || 'Focus on daily moisture barrier hydration and daily SPF 50 sunscreen protection.',
        suggestedIngredients: parsed.suggestedIngredients || ['Niacinamide', 'Hyaluronic Acid', 'Ceramides'],
      };
    } catch (err) {
  ...
  const serum = catalogProducts.find((p) => p.category === 'serum' && p.skinType.includes(input.skinType)) || catalogProducts[1];
  ...
  const exfoliantOrNight = catalogProducts.find((p) => p.category === 'exfoliant' || (p.category === 'serum' && p.usage === 'evening')) || catalogProducts[4];
...
If relevant, mention 1-2 catalog products by exact name.`;
  ...
  if (queryLower.includes('niacinamide') && queryLower.includes('vitamin c')) {
    ...
    responseText = 'For acne-prone or oily skin, Salicylic Acid (BHA 2%) penetrates deep into pores to dissolve excess sebum, while Niacinamide reduces inflammatory redness. Try our **Clarifying BHA 2% Salicylic Acid Exfoliant** 2-3 nights per week!';
    suggested = catalogProducts.filter((p) => p.category === 'exfoliant' || p.name.includes('Niacinamide'));
  } else if (queryLower.includes('hyaluronic acid') || queryLower.includes('dry')) {
    responseText = 'Hyaluronic Acid is a moisture magnet capable of holding up to 1,000 times its weight in water! Apply our **Multi-Weight Hyaluronic Acid Hydrating Booster** onto damp skin before locking it in with moisturizer.';
    suggested = catalogProducts.filter((p) => p.name.includes('Hyaluronic') || p.category === 'moisturizer').slice(0, 2);
  } else {
